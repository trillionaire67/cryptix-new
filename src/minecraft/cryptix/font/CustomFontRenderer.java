...
public class CustomFontRenderer extends FontUtil {
    FontUtil.CharData[] boldChars = new FontUtil.CharData[256],
            italicChars = new FontUtil.CharData[256],
            boldItalicChars = new FontUtil.CharData[256];
    ...
    public int drawString(String text, double x2, float y2, int color, boolean shadow) {
    	if(shadow) {
    		float shadowWidth = this.drawString(text, x2 + 0.9f, (double) y2 + 0.5f, color, true, 8.3f);

            return (int) Math.max(shadowWidth, this.drawString(text, x2, y2, color, false, 8.3f));
    	}
        return (int) this.drawString(text, x2, y2, color, shadow, 8.3f);
    }
    ...
    public float drawStringNoGL(String text,double x,double y,int color,boolean shadow,float kerning){
        if(text==null||text.isEmpty())return 0f;
        x-=1.0;
        if((color&0xFC000000)==0)color|=0xFF000000;
        if(shadow)color=((color&0xFCFCFC)>>2)|(color&0xFF000000);
    ...
    public float drawSmoothString(String text, double x, double y, int color, boolean shadow) {
        ...
        FontUtil.CharData[] currentData = this.charData;
    ...
    public double getStringWidth(String text) {
        ...
        final FontUtil.CharData[] data = this.charData;
        final int dataLen = data.length;
        final float offset = this.charOffset;
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (c != 167) {
                if (c < dataLen) {
                    width += (data[c].width - 8.3f + offset);
                }
            } else {
                i++;
            }
    ...
    public double getStringWidth(String text, float kerning) {
        ...
        float width = 0;
        FontUtil.CharData[] currentData = charData;
