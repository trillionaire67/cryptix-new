...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            if (this.itemToRender != null)
            {
                ...
                this.renderItem(abstractclientplayer, Client.instance.moduleManager.scaffold.isToggled() && Client.instance.moduleManager.scaffold.spoof.getBoolean() && Client.mc.thePlayer.inventory.getStackInSlot(Client.instance.moduleManager.scaffold.lastSlot) != null ? Client.mc.thePlayer.inventory.getStackInSlot(Client.instance.moduleManager.scaffold.lastSlot) : this.itemToRender, ItemCameraTransforms.TransformType.FIRST_PERSON);
            }
    ...
    private void doBlockAnim(String mode, float f, float f1) {
    	float swingProg = (float) Math.sin(Math.sqrt(f1) * Math.PI);
    	switch(mode.toLowerCase()) {
	    	...
	    	case "exhibition":
				...
				float rotationFactor = swingProg * 20.0f;
				float rotatedX = swingProg / 2.0f;
				GL11.glRotated(-rotationFactor, rotatedX, 0.0, 9.0);
				rotationFactor = swingProg * 35.0f;
				float rotatedY = swingProg / 2.5f;
				GL11.glRotated(-rotationFactor, 0.8f, rotatedY, 0.0);
				this.doBlockTransformations();
	    		break;
	    	case "spin":
	    		this.transformFirstPersonItem(f, 0.0F);
				this.doBlockTransformations();
				GL11.glRotatef(rotate * 2, rotate * 2, 0, rotate * 2);
	    	...
	    	case "slide":
	    		...
	    		GL11.glRotated((-32 * swingProg), 1, 0, -0.5);
				this.doBlockTransformations();
	    		break;
	    	case "none":
	    		this.transformFirstPersonItem(f, 0.0F);
